import {mainInputClasses} from "@/shared/styles";
import {memo} from "react";
import {MainInputProps} from "@/shared/types";
import InputError from "@/shared/UI-kit/errors/InputError";

function MainInput(
    {
        label,
        id,
        type = 'text',
        icon,
        required = false,
        placeholder,
        error,
        ...rest
    }: MainInputProps) {

    return (
        <div>
            {label && (
                <label htmlFor={id} className="block mb-2 text-sm font-medium text-gray-500 dark:text-gray-300">
                    {label}
                </label>
            )}

            <div className="relative">
                {icon && (
                    <div className="pointer-events-none absolute inset-y-0 left-0 text-gray-500 dark:text-gray-300 flex items-center pl-3">
                        {icon}
                    </div>
                )}

                <input
                    id={id}
                    name={id}
                    type={type}
                    required={required}
                    placeholder={placeholder}
                    className={`${mainInputClasses.trim()} ${icon ? 'pl-10 pr-4' : 'px-4'}`}
                    {...rest}
                />
            </div>

            <InputError error={error} />
        </div>
    )
}

export default memo(MainInput);